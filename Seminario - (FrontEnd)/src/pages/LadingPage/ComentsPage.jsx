...
export const ComentsPage = () => {
    ...
    const getComents = async () => {
        try {
            const { data } = await axios('http://localhost:3200/comment/getAll', config);
            if (data)
    ...
    const addComment = async () => {
        try {
            ...
            const { data } = await axios.post('http://localhost:3200/comment/add', comment);
            Swal.fire({
    ...
    const search = async () => {
        try {
            ...
            const { data } = await axios.get('http://localhost:3200/comment/search', { params: { name } });
            if (data) {
                setComment(data.comments);
            }
            console.log(data);
    ...
    const deleteComment = async (id) => {
        try {
          Swal.fire({
            ...
            if (result.isDenied) {
              const { data } = await axios.delete(`http://localhost:3200/comment/delete/${id}` ,config);
              console.log(data);
              Swal.fire({
